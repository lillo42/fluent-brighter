...
public class RmqSubscriptionBuilder
{
    ...
    private QueueType _queueType = Paramore.Brighter.MessagingGateway.RMQ.Async.QueueType.Classic;

    /// <summary>
    /// Sets the queue type (classic or quorum).
    /// </summary>
    /// <param name="queueType">The RabbitMQ queue type.</param>
    /// <returns>The current builder instance for fluent chaining.</returns>
    public RmqSubscriptionBuilder QueueType(QueueType queueType)
    {
    ...
    /// <summary>
    /// Sets the queue type as classic
    /// </summary>
    /// <returns>The current builder instance for fluent chaining.</returns>
    public RmqSubscriptionBuilder AsClassicQueue() =>
        QueueType(Paramore.Brighter.MessagingGateway.RMQ.Async.QueueType.Classic);
    
    /// <summary>
    /// Sets the queue type as quorum
    /// </summary>
    /// <returns>The current builder instance for fluent chaining.</returns>
    public RmqSubscriptionBuilder AsQuorumQueue() =>
        QueueType(Paramore.Brighter.MessagingGateway.RMQ.Async.QueueType.Quorum);
